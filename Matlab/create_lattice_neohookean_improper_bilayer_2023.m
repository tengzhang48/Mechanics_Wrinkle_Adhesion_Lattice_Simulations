function create_lattice_neohookean_improper_bilayer_2023
...
nx = fix(337);              % number of lattice along x direction
ny = fix(300);              % number of lattice along y direction
...
nxy = (ny+1)*nx;            % total atom number for x-peridical boundary condition
...
fn = ['lattice-bilayer-improper-a',ac,'-Lx',Lxc,'-Ly',Lyc,'-hc',nhc];
...
b_s = [-nx*a/2 nx*a/2; -ny*a/2*1.1 ny*a/2*2.0; -50. 50.];       % box
